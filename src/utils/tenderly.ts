...
import { CONTRACT_ABI, CONTRACT_ADDRESS, ERC20_ABI, TransactionSimulation, parseTokenAmount } from './web3';
...
export function generateERC20TransferData(to: string, amount: bigint): string {
  // Create an interface from the ERC20 ABI
...
/**
 * Generate custom contract fetchToken transaction data
 * @param tokenAddress ERC20 token address
 * @param amount Amount to fetch (in token units)
 * @returns Encoded transaction data
 */
export function generateFetchTokenData(tokenAddress: string, amount: bigint): string {
  // Create an interface from the CONTRACT_ABI
  const iface = new ethers.Interface(CONTRACT_ABI);
  
  // Encode the fetchToken function call
  const data = iface.encodeFunctionData('fetchToken', [tokenAddress, amount]);
...
export function isTenderlyVirtualTestNet(chainId: number): boolean {
  // This is a placeholder - in a real implementation, you'd have a list of Tenderly Virtual TestNet chain IDs
  // Include our custom devnet chain ID (which is the same as Sepolia for compatibility)
  return chainId === 73571 || chainId === 11155111 || !!TENDERLY_VIRTUAL_TESTNET_RPC;
}
...
/**
 * Simulate an ERC20 token transfer
...
export async function simulateERC20Transfer(
  ...
  try {
    ...
    // Generate the ERC20 transfer data
    const data = generateERC20TransferData(to, tokenAmount);
    
    // Simulate the transaction
    return simulateTransactionWithTenderly(
      from,
      tokenAddress, // The token contract address
      BigInt(0), // No ETH value for ERC20 transfers
      data,
      chainId
    );
  } catch (error: any) {
    console.error('Error simulating ERC20 transfer:', error);
    return {
      success: false,
      error: error.message || 'Error simulating ERC20 transfer',
    };
  }
}

/**
 * Simulate a fetchToken operation with custom contract
 * @param from Sender address
 * @param tokenAddress Token contract address
 * @param amount Amount to transfer (in token units)
 * @param decimals Token decimals
...
export async function simulateFetchTokens(
  ...
  try {
    ...
    // Generate the fetchToken data
    const data = generateFetchTokenData(tokenAddress, tokenAmount);
    ...
    console.error('Error simulating fetchToken:', error);
    return {
      success: false,
      error: error.message || 'Error simulating fetchToken',
    };
  }
}
...
async function simulateWithTenderlySdk(
  ...
  try {
    // Use latest block number for simulation
    // In a real implementation, you might want to fetch the actual latest block number
    ...
    const simulation = await tenderly.simulator.simulateTransaction({
      ...
      // Note: The SDK TypeScript interface might not expose all available options
      // For full options, see the Tenderly API documentation
    });
    ...
    if (simulation && simulation.status) {
      ...
      // Calculate gas cost
      // Note: In a real implementation, you'd use the actual gas price from the simulation
